...
const EmpCheckIn = () => {
    ...
    const loadRegions = async () => {
        try{
            ...
            if (res.data.length > 0) {
              const defaultRegion = res.data[0]; // first region
  ...
  /* ===== Load attendance for region ===== */
  const loadAttendance = async (regionId) => {
    try{
        const res = await hrApi.getCheckIn(regionId);
  ...
  const handleAcceptClick = async (attendanceId) => {
    try {
      await hrApi.verifyAttendance(attendanceId, "Confirmed" ,"CheckIn");
      setAttendance(prev => 
        prev.filter(a => a.attendanceId !== attendanceId)
      );
    }catch(err){
      alert("Failed to approve Attendace");
      console.error(err);
    }
  };
  const handleRejectClick = async (attendanceId) => {
    try {
      await hrApi.verifyAttendance(attendanceId, "Rejected" ,"CheckIn");
      setAttendance(prev => 
        prev.filter(a => a.attendanceId !== attendanceId)
      );
    }catch(err){
      alert("Failed to Rejection Attendace");
      console.error(err);
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {attendance.map((a) => (
          ...
          >
            ...
            <p>Location : {a.checkInLocationAddress}</p>
            <p>Time : {a.checkInTime}</p>
